    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            if (response.data && (response.data.success || response.data.id || response.status === 200)) {
                ...
                // Force UI update by creating a new file input element
                const fileInput = document.getElementById('image');
                if (fileInput) {
                    fileInput.value = '';
                }
            } else {
                setMessage({ text: "Failed to add item. Server response: " + JSON.stringify(response.data), type: "error" })
            }
        } catch (error) {
            console.error("API Error details:", error)
            if (error.response) {
                setMessage({ 
                    text: `Server error: ${error.response.status} - ${error.response.data.message || 'Unknown error'}`, 
                    type: "error" 
                })
